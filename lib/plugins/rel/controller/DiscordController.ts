...
export class DiscordController{
    ...
    async onMessage(message: Message){
        ...
        const [_, category, actionLiteral] : [string, string, string]  = message.content.match(DiscordController.REGEX) as [string ,string,string] 
    ...
    matchesMessage(message: Message): boolean {
        ...
        const matchesJudgementPattern = !!message?.content?.match(DiscordController.REGEX)
    ...
    async respond(message: Message, judgedUsername: string, points: number, category: string): Promise<void>{
